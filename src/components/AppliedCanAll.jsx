...
function AppliedCanAll() {
    ...
    const filteredData = data.filter(item => {
        const jobDetail = jobDetails[item.jobId]; // Get related job details
        const manualJobID = jobDetail?.manualJobID && jobDetail.manualJobID !== "0" ? jobDetail.manualJobID : null;
        ...
        return (
            ...
            (manualJobID && String(manualJobID).toLowerCase().includes(searchTerm.toLowerCase()))
        );
    });
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
                    {filteredData.map((item) => {
                        ...
                        return (
                            <div
                                key={item.id}
                                className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 text-left"
                            >
                                <h2 className="text-lg font-bold text-gray-900 mb-2 font-display">{item.customerName}</h2>
                                <p className="text-gray-600 mb-1 font-display">
                                    <strong>Job ID:</strong>{' '}
                                    {jobDetail?.manualJobID && jobDetail.manualJobID !== "0"
                                        ? jobDetail.manualJobID
                                        : item.jobId}
                                </p>
                                <p
